...
public class ATM {
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (slt == 0) { // 0 입력시 종료
			...
			} else if (slt == 2 && log == 1) { // 2 선택, 로그인 시 회원탈퇴 가능
				...
				while (true) {
					...
					for (int i = 0; i < 5; i++) {
						if ((arID[i] == logid) && (arPW[i] == logpw)) { // 저장된 아이디와 입력 받은 아이디가 동일할 경우 위치 기억(패스워드 동일)
							chkid = i;
							chkpw = i;
						}
					}
			...
			} else if (slt == 3 && log == -1) { // 3 입력, 로그아웃 상태일 떄 로그인 가능
				...
				while (true) {
					...
					System.out.println("PW 입력:");
					int logpw = sc.nextInt();
					for (int i = 0; i < 5; i++) { // 저장된 아이디와 입력 받은 아이디가 동일할 경우 위치 기억(패스워드 동일)
			...
			} else if (slt == 4 && log == 1) { // 4 입력, 로그인 상태일 떄 로그아웃 가능
				while (true) {
					int chkid = 100, chkpw = 1000;
					System.out.println("ID 입력:");
					int logid = sc.nextInt();
			...
			} else if (slt == 5 && log == 1) { // 5 입력, 로그인 상태일 떄 입금 가능
			...
			} else if (slt == 6 && log == 1) { // 6 입력, 로그인 상태일 떄 이체 가능
			...
			} else if (slt == 7 && log == 1) { // 7 입력, 로그인 상태일 떄 잔액조회 가능
